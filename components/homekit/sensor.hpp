  ...
  namespace homekit
  {
    ...
    public:
      ...
      void setup() {
        ...
        else if (std::equal(device_class.begin(), device_class.end(), strdup("distance"))) {
          service = hap_serv_create(HAP_SERV_UUID_DISTANCE_SENSOR);
          hap_serv_add_char(service, hap_char_distance_create(sensorPtr->state));
        }
